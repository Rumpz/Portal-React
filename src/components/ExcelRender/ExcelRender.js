...
export default class ExcelRender extends React.Component {
  ...
  /* convert state to workbook */
    ...
    XLSX.writeFile(wb, 'downloaded_File_please_rename.xlsx');
  }
  render () {
    const excelTable = this.state.data.length > 0 && this.state.cols.length > 0
      ? <ExcelTable data={this.state.data} cols={this.state.cols} />
      : null;
    return (
